...
function CloudScene({ scrollY, sectionIndex }: CloudSceneProps) {
  ...
  const paletteB =
    skyPalettes[sectionIndex + 1] || skyPalettes[sectionIndex] || skyPalettes[0];
  ...
  useFrame((state) => {
    ...
    if (cloudsRef.current) {
      ...
      cloudsRef.current.rotation.x =
        ...
        scrollY * 0.0001;
      const groupScale =
        1 + Math.sin(time * 0.06) * 0.01 + Math.cos(time * 0.089) * 0.008;
    ...
    function animateCloudAdvanced(
      ...
      if (
        ...
        const primaryDrift =
          Math.sin(time * 0.04 + personalityOffset) * 1.8 * parallaxFactor;
        const secondaryDrift =
          Math.cos(time * 0.076 + personalityOffset * 0.5) * 0.8 * parallaxFactor;
        const crossWind = Math.sin(time * 0.032 + personalityOffset * 0.4) * 0.4;

        const breathingY =
          Math.cos(time * 0.058 + personalityOffset) * 0.6 * parallaxFactor;
        const floatY =
      ...
      <div className="text-center max-w-xl mx-auto px-4 relative z-10">
        ...
        <div
          ...
          style={{
            ...
            transform: isActive ? `scaleX(${dividerScale})` : "scaleX(0)",
            transition: isActive
              ? "opacity 0.3s ease-out, transform 0.3s ease-out"
...
export default function BlueSkyLanding() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const active = offsets.findIndex(
        (offset) =>
          offset > window.innerHeight * -0.5 && offset < window.innerHeight * 0.5,
      );
  ...
  return (
    // This is the main flex container for the entire page.
    // It ensures that its children (main content + footer) stack vertically
    // and that the main content pushes the footer to the bottom.
    <div
      className="flex flex-col min-h-screen
                 bg-gradient-to-br from-blue-50/50 via-indigo-50/20 to-white
                 overflow-auto relative snap-y snap-mandatory"
    >
      {/* Clouds Toggle Button - fixed and independent of scrolling content */}
      ...
      {/* Three.js Canvas - Conditionally rendered and fixed */}
      {showClouds && (
      ...
      {/* Fixed background gradient overlay for visual effect */}
      ...
      {/*
        ...
        The `flex-grow` class is crucial here; it makes this div expand
        to fill any available vertical space, pushing the Footer down.
      */}
      <main className="flex-grow relative z-10 container mx-auto px-6 py-20">
        {/* Your intro section (Blue Sky title, quotes, button etc.) */}
        <div className="flex flex-col items-center justify-center min-h-screen max-w-5xl mx-auto">
          <div className="space-y-11 text-center">
            ...
            >
              ...
              >
                A journey woven with threads of longing, where the sky&apos;s embrace
                unveils the heart&apos;s quiet dreams.
              </p>
              <p
      ...
      </main> {/* End of the main, growable content area */}

      {/* The Footer component, positioned correctly after the growable content */}
      <Footer />
    </div>
  );
}